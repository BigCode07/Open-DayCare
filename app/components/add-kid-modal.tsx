...
export type AddKidFormHandle = {
  save: () => Kid | null;
  reset: () => void;
};

const AddKidForm = forwardRef<AddKidFormHandle>(function AddKidForm(_props, ref) {
...
export default function AddKidModal({
  ...
  const formRef = useRef<AddKidFormHandle>(null);
  ...
  const handleSave = () => {
    const kid = formRef.current?.save();
    if (kid) {
      onSaved(kid);
